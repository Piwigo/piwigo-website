...
;';
  ...
  $template->assign(
    array(
      ...
      'AUTHORIZE_ROBOTS' => $is_production,
      'TRACK_ANALYTICS' => ($is_production and (!isset($_GET['porg']) or 'missing-account' != $_GET['porg'])),
      ...
      'URL_PARAM_SEPARATOR' => conf_get_param('porg_url_rewrite', false) ? '?' : '&',
...
function porg_init_redirect_https()
{
  ...
  if ($is_bot or $is_from_piwigodotorg) {
    ...
    if ($_SERVER['REQUEST_METHOD'] !== 'POST' and (!isset($_SERVER['HTTPS']) or $_SERVER['HTTPS'] != 'on')) {
      global $logger;
      $logger->info(__FUNCTION__ . ', redirect (' . ($is_bot ? 'bot' : '') . ' ' . ($is_from_piwigodotorg ? 'from_piwigo.org' : '') . ')');
