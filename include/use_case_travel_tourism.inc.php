...
foreach ($all_logos as $logo) {
  ...
  if (isset($logo_tags['useCase']) && $logo_tags['useCase'] === 'tourism') {
    $tourism_logos[] = $logo;
  }
}

$all_testimonials = get_ressources('testimonials');
$tourism_testimonials = array();

foreach ($all_testimonials as $testimonial) {
  $testimonial_tags = get_tags_of($testimonial['id']);

  if (isset($testimonial_tags['useCase']) && $testimonial_tags['useCase'] === 'tourism') {
    $item_content = array(
      'id' => $testimonial['id'],
    ...
    $tourism_testimonials[] = array_merge($item_content, $testimonial_tags);
  }
}

$template->assign(
  array(
    'tourism_logos' => $tourism_logos,
    'tourism_testimonials' => $tourism_testimonials,
  )
);
