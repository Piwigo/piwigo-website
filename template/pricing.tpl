...
<div id="card-selfhosted" class="selfhosted-card-container d-none-card">
    {include file="template/include/card/global_text_image.tpl" title={'pricing title selfhosted card'|translate}
    desc={'pricing selfhosted description card'|translate} image="{$PORG_ROOT_URL}images/pricing/selfhosted_deploy.png"
    }
</div>
...
<section class="faq_pricing_container container">
    ...
    <div class="row faq_pricing_grid g-0">
        {foreach from=$faqs item=faq}
            {include file="template/include/card/faq_item.tpl" faq_id=$faq.id faq_question=$faq.question faq_answer=$faq.answer}
        {/foreach}
    </div>
</section>
