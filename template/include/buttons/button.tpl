...
<a href="{$href|default:'#'}" class="{$classes}{if isset($class)} {$class}{/if}" {if isset($target)}target="{$target}" {/if} {if isset($target) && $target eq '_blank'}rel="noopener" {/if}><span>{$label|default:$default_label}</span></a>